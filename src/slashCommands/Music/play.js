...
if (query.startsWith('https://soundcloud.com/')) {
    ...
    await  getPreview(`${query}`, {
           ...
           await infos.searchArtist(data.artist).then(async nameinfo => { 
            ...
            await   interaction.editReply({
             embeds: [new MessageEmbed() .setColor(client.embedColor)
               .setDescription(`Queued [${data.title}](${data.link}) by [${data.artist}](${nameinfo[0].external_urls.spotify}) [\`${interaction.member.user.tag}\`]`)]
           }).catch(() => {});
           const s = await player.search({
             query: infosearch,
           source : 'youtube'
 ...
 let msg =   await   interaction.editReply({ embeds : [thing]})
 if (player && player.state !== "CONNECTED") player.connect();
...
return;
  ...
  //playlist
    ...
    if(sres.tracks.items > 1){
     ...
     await   interaction.editReply({
      embeds: [new MessageEmbed() .setColor(client.embedColor)
        .setDescription(`Queued [${sres.tracks.items[0].name}](${sres.tracks.items[0].external_urls.spotify}) by [${sres.tracks.items[0].artists[0].name}](${sres.tracks.items[0].artists[0].external_urls.spotify}) [\`${interaction.member.user.tag}\`]`)]
    }).catch(() => {});
